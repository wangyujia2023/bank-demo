...
class ReportService:
    ...
    async def business_overview(self):
        stats, asset_dist, lifecycle_dist, city_dist, active_dist = await asyncio.gather(
            execute_one("""
                SELECT
    ...
    async def transaction_report(self):
        channel_stats, event_dist, hourly_trend = await asyncio.gather(
            execute_query("""
                SELECT channel,
                       COUNT(*) AS tx_count,
    ...
    async def risk_report(self):
        risk_stats, risk_by_asset, risk_by_city, credit_dist = await asyncio.gather(
            execute_one("""
                SELECT
